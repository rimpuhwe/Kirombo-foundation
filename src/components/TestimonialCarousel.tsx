...
const TestimonialCarousel: React.FC = () => {
  ...
  return (
    <div className="w-full max-w-5xl mx-auto mt-10 mb-10">
      <h3 className="text-3xl md:text-4xl font-extrabold mb-8 text-orange text-left">Voices from the Program</h3>
      {/* Desktop: 3 at a time, Mobile: 1 at a time */}
      <div className="hidden md:block relative">
        <Carousel opts={{ loop: true, align: 'start' }} setApi={handleSetApi} className="w-full">
          <CarouselContent>
            {testimonials.map((t, idx) => (
              <CarouselItem key={idx} className="md:basis-1/2 px-4 flex flex-col items-start justify-center h-full">
                ...
                <span className="block text-left text-base font-semibold text-primary mt-2 w-full truncate">{t.author}</span>
              </CarouselItem>
            ))}
          </CarouselContent>
      ...
      <div className="block md:hidden">
        <Carousel opts={{ loop: true }} setApi={handleSetApi} className="w-full">
          <CarouselContent>
            {testimonials.map((t, idx) => (
              <CarouselItem key={idx} className="w-full px-4 flex flex-col items-start justify-center h-full">
                <span className="text-orange-500 text-6xl mb-4">
                  <i className="fa fa-quote-left" aria-hidden="true"></i>
                </span>
                <blockquote className="italic text-xl text-left text-gray-700 mb-4 leading-relaxed w-full max-w-full">
